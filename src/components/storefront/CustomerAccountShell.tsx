...
export function CustomerAccountShell({ hash, customer, active, title, description, children }: { hash: string; customer: AccountCustomer; active: string; title: string; description?: string; children: React.ReactNode }) {
  ...
  return <div className="mx-auto max-w-6xl px-1 py-2"><div className="mb-7 border-b border-slate-200 pb-5"><h1 className="text-3xl font-extrabold tracking-tight text-slate-900">{title}</h1>{description && <p className="mt-1 text-sm text-slate-500">{description}</p>}</div><div className="grid gap-8 lg:grid-cols-[230px_1fr]"><aside className="h-fit rounded-2xl border border-slate-200 bg-white p-3 shadow-sm"><div className="border-b border-slate-100 px-3 pb-3"><div className="text-xs uppercase tracking-wider text-slate-400">Halo,</div><div className="font-bold text-slate-900">{customer.name}</div></div><nav className="space-y-1 pt-3">{menu.map(([key, label, href]) => <Link key={key} href={href} className={`block rounded-lg px-4 py-2.5 text-sm font-semibold ${active === key ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>{label}</Link>)}<Link href={`/storefront/${hash}`} className="block rounded-lg px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50">Lanjut Belanja</Link><CustomerLogoutButton hash={hash} /></nav></aside><main>{children}</main></div></div>;
}
