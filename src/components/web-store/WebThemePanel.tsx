...
import { defaultTheme, normalizeTheme, THEME_PRESETS, FONT_OPTIONS, normalizeChrome, type WebTheme } from '@/lib/webTheme';
 ...
 *
 * Logo/banner & header/footer GLOBAL sengaja TIDAK dikelola di sini: halaman
 * kanvas memakai blok StoreHeader/StoreFooter/Banner per halaman (Page Builder).
 * Nilai chrome lama tetap dipertahankan saat simpan (halaman klasik seperti
 * orders/akun masih memakainya) tapi tidak diedit dari panel ini.
 */
export default function WebThemePanel({
  ...
  async function saveTheme() {
    ...
    setErr('');
    try {
      // Chrome (header/footer halaman klasik) dipertahankan apa adanya —
      // tidak diedit di sini, tapi jangan sampai terhapus saat simpan tema.
      const chrome = normalizeChrome(initialChrome ?? null);
      await upsertWebStore(token, {
        store_id: storeId,
        store_name: storeName,
        settings: { theme, chrome },
      });
      setOk('Tema disimpan. Semua halaman Page Builder memakai warna/font ini.');
      onSaved?.(theme);
    } catch (e: any) {
