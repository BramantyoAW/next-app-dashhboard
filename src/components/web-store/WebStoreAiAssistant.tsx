...
export function WebStoreAiAssistant({ context, onApply }: { context: unknown; onApply: (changes: AiAssistantResult['changes']) => void }) {
  ...
  async function send() {
    ...
    try {
      ...
      const res = await askWebStoreAssistant(token, { message: text, context, history });
      setMessages((m) => [...m, { role: 'assistant', text: res.aiWebStoreAssistant.reply, result: res.aiWebStoreAssistant }]);
    } catch (e: any) {
    ...
    <>
      ...
      {open && (
        <section className="fixed bottom-0 right-0 z-[51] flex h-[min(720px,90vh)] w-full max-w-md flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:bottom-6 sm:right-6 sm:rounded-2xl">
          <header className="flex items-center justify-between bg-slate-900 px-5 py-4 text-white"><div className="flex items-center gap-2"><Bot size={20} /><div><div className="font-bold">Design dengan AI</div><div className="text-[11px] text-slate-300">Tanya ide atau minta perubahan tampilan</div></div></div><button type="button" onClick={() => setOpen(false)} aria-label="Tutup"><X size={18} /></button></header>
