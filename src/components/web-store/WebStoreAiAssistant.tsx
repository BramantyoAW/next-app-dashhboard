  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} className="fixed bottom-6 right-6 z-40 inline-flex items-center gap-2 rounded-full bg-slate-900 px-4 py-3 text-sm font-bold text-white shadow-xl hover:bg-slate-700">
        <Sparkles size={16} /> Manage OmBot Ai
      </button>
      ...
      {open && (
        <section className="fixed bottom-0 right-0 z-[51] flex h-[min(720px,90vh)] w-full max-w-md flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:bottom-6 sm:right-6 sm:rounded-2xl">
          <header className="flex items-center justify-between bg-slate-900 px-5 py-4 text-white"><div className="flex items-center gap-2"><Bot size={20} /><div><div className="font-bold">Manage OmBot Ai</div><div className="text-[11px] text-slate-300">Scope: {scope} · {historyLoading ? 'Memuat riwayat…' : 'riwayat tersimpan'}</div></div></div><div className="flex items-center gap-2"><button type="button" disabled={!webStoreId} onClick={async () => { const token = localStorage.getItem('token') || ''; if (token && webStoreId) { await clearAiHistory(token, webStoreId).catch(() => {}); setMessages([]); } }} aria-label="Hapus riwayat" title="Hapus riwayat" className="rounded-lg p-1.5 hover:bg-white/10"><Trash2 size={16} /></button><button type="button" onClick={() => setOpen(false)} aria-label="Tutup" className="rounded-lg p-1.5 hover:bg-white/10"><X size={18} /></button></div></header>
